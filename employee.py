...
class Employee:
    ...
    def set_bonus_pay(self):
        if (self.hourly_payment != None):
            self.total_pay = self.work_hour * self.hourly_payment

        if (self.hourly_payment != None and self.contract_payment != None):
            self.total_pay = self.work_hour * self.hourly_payment
    ...
    def get_commission_pay(self):
        ...
        if (self.contract_payment != None): #its not bonus
            self.total_pay +=self.contract_payment

    def get_pay(self):
        self.set_bonus_pay()
        self.get_commission_pay()
    ...
    def __str__(self):
         return f"{self.name} works on a {self.checkType()}.  Their total pay is {self.total_pay}."
